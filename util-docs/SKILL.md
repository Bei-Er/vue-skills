...
在 `src/` 目录下（排除 utils 自身文件）搜索 import 语句，统计引用次数。

**搜索规则：**
- 使用每个导出函数名（如 `formatDate`、`getCookie`）作为搜索关键字
- 匹配 `import { funcName }` 和 `import funcName` 两种形式
- 排除 utils 源文件本身
- 只搜索 `src/` 目录下的 `.vue`、`.js`、`.ts` 文件
- 同一文件中 import 多个函数时，每个函数分别计数
...
（标题格式：中文名称(引用次数)，从目录名、函数用途推断中文名称）
